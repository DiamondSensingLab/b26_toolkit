...
class DressedStateCoupling(PulseBlasterBaseScript):
    """
    This script is used to measure dressed-state coupling between NV and electron spin
    To symmetrize the sequence between the 0 and +/-1 state we reinitialize every time.

    ==> last edited by Ziwei Qiu 8/30/2017
    """
    _DEFAULT_SETTINGS = [
        Parameter('mw_pulses', [
            Parameter('mw_power', 3, float, 'microwave power in dB'),
        ...
        Parameter('spin_lock_times', [
            Parameter('time_before_spin_lock', 50, float, 'time between the first pi/2 pulse and spin-lock turning on'),
            Parameter('min_time', 500, float, 'minimum spin_lock time between pi/2 pulses'),
            Parameter('max_time', 10000, float, 'maximum spin_lock time between pi/2 pulses'),
            Parameter('time_step', 500,
                      [10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000, 500000],
                      'time step increment of time between pi/2 pulses (in ns)')
        ]),
        # Parameter('decoupling_seq', [
        #     Parameter('type', 'spin_echo', ['spin_echo', 'XY4', 'XY8', 'CPMG'],
    ...
    def _function(self):
        ...
        self.data['norm_control'] = None
        self.data['norm_exp'] = None
        self.data['control_err'] = None
        self.data['exp_err'] = None
        ...
        super(DressedStateCoupling, self)._function(self.data)
        ...
        self.data['norm_control'] = 2.*(- self.data['counts'][:, 0] + self.data['counts'][:,1]) / (self.data['counts'][:,1] + self.data['counts'][:, 0])
        self.data['norm_exp'] = 2.*(- self.data['counts'][:, 2] + self.data['counts'][:,3]) / (self.data['counts'][:,3] + self.data['counts'][:, 2])

        # error propagation starting with shot noise for each trace:
        self.data['control_err'] = 2*(self.data['counts'][:,1]*self.data['counts'][:, 0])/np.square(self.data['counts'][:,1] + self.data['counts'][:, 0])*np.sqrt(np.square(self.data['shot_noise'][:, 0]) + np.square(self.data['shot_noise'][:, 1]))
        self.data['exp_err'] = 2*(self.data['counts'][:,3]*self.data['counts'][:, 2])/np.square(self.data['counts'][:,3] + self.data['counts'][:, 2])*np.sqrt(np.square(self.data['shot_noise'][:, 2]) + np.square(self.data['shot_noise'][:, 3]))
    ...
    def _create_pulse_sequences(self):
        ...
        spin_lock_time_list = range(int(self.settings['spin_lock_times']['min_time']), int(self.settings['spin_lock_times']['max_time']),self.settings['spin_lock_times']['time_step'])

        # ignore the sequence if the mw-pulse is shorter than 15ns (0 is ok because there is no mw pulse!)
        spin_lock_time_list = [x for x in spin_lock_time_list if x == 0 or x >= 15]
        print('spin_lock_time_list', spin_lock_time_list)
        ...
        gap_before_spin_lock = self.settings['spin_lock_times']['time_before_spin_lock']
        ...
        for spin_lock_time in spin_lock_time_list:
            # no RF control:
            pulse_sequence = \
            [
                Pulse(microwave_channel_1, laser_off_time, pi_half_time),
                Pulse(microwave_channel_2, laser_off_time + pi_half_time + gap_before_spin_lock, spin_lock_time),
                Pulse(microwave_channel_1, laser_off_time + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock , pi_half_time)
            ]

            end_of_first_control = laser_off_time + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock + pi_half_time

            pulse_sequence += [
                 Pulse('laser', end_of_first_control + delay_mw_readout, nv_reset_time),
                 Pulse('apd_readout', end_of_first_control + delay_mw_readout + delay_readout, meas_time),
                 ]

            start_of_second_control = end_of_first_control + delay_mw_readout + nv_reset_time + laser_off_time

            pulse_sequence += \
            [
                Pulse(microwave_channel_1, start_of_second_control, pi_half_time),
                Pulse(microwave_channel_2, start_of_second_control + pi_half_time + gap_before_spin_lock, spin_lock_time),
                Pulse(microwave_channel_1, start_of_second_control + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock , three_pi_half_time)
            ]

            end_of_second_control = start_of_second_control + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock + three_pi_half_time

            pulse_sequence += [
                Pulse('laser', end_of_second_control + delay_mw_readout, nv_reset_time),
                Pulse('apd_readout', end_of_second_control + delay_mw_readout + delay_readout, meas_time)
            ]

            # RF on

            start_of_first_exp = end_of_second_control + delay_mw_readout + nv_reset_time + laser_off_time
            pulse_sequence += \
            [
                Pulse(microwave_channel_1, start_of_first_exp, pi_half_time),
                Pulse(microwave_channel_2, start_of_first_exp + pi_half_time + gap_before_spin_lock, spin_lock_time),
                Pulse('RF_switch', start_of_first_exp + pi_half_time + gap_before_spin_lock, spin_lock_time),
                Pulse(microwave_channel_1, start_of_first_exp + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock, pi_half_time)
            ]

            end_of_first_exp =  start_of_first_exp + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock + pi_half_time

            pulse_sequence += [
                 Pulse('laser', end_of_first_exp + delay_mw_readout, nv_reset_time),
                 Pulse('apd_readout', end_of_first_exp + delay_mw_readout + delay_readout, meas_time),
                 ]

            start_of_second_exp = end_of_first_exp + delay_mw_readout + nv_reset_time + laser_off_time

            pulse_sequence += \
            [
                Pulse(microwave_channel_1, start_of_second_exp, pi_half_time),
                Pulse(microwave_channel_2, start_of_second_exp + pi_half_time + gap_before_spin_lock, spin_lock_time),
                Pulse('RF_switch', start_of_second_exp + pi_half_time + gap_before_spin_lock, spin_lock_time),
                Pulse(microwave_channel_1, start_of_second_exp + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock, three_pi_half_time)
            ]

            end_of_second_exp = start_of_second_exp + pi_half_time + gap_before_spin_lock + spin_lock_time + gap_before_spin_lock + three_pi_half_time

            pulse_sequence += [
                Pulse('laser', end_of_second_exp + delay_mw_readout, nv_reset_time),
                Pulse('apd_readout', end_of_second_exp + delay_mw_readout + delay_readout, meas_time)
            ]

            pulse_sequences.append(pulse_sequence)

        print('number of sequences before validation ', len(pulse_sequences))
        return pulse_sequences, self.settings['num_averages'], spin_lock_time_list, meas_time
    ...
    def _plot(self, axislist, data = None):
        ...
        if data['norm_control'] is not None and data['norm_exp'] is not None and data['control_err'] is not None and data['exp_err'] is not None:

            # axislist[0].plot(tau, data['norm_echo'], 'b')
            # axislist[0].hold(True)
            # axislist[0].plot(tau, data['norm_deer'], 'r')

            axislist[0].errorbar(tau, data['norm_control'], data['control_err'])
            axislist[0].hold(True)
            axislist[0].errorbar(tau, data['norm_exp'], data['exp_err'])

            # tauinterp = np.linspace(np.min(tau),np.max(tau),100)
            # axislist[0].plot(tauinterp, exp_offset(tauinterp, fits_echo[0], fits_echo[1], fits_echo[2]),'b:')
            # axislist[0].plot(tauinterp, exp_offset(tauinterp, fits_deer[0], fits_deer[1], fits_deer[2]), 'g:')
            # axislist[0].set_title('T2 decay times (simple exponential, p = 1): echo={:2.1f} ns, deer = {:2.1f} ns'.format(fits_echo[1],fits_deer[1]))
            axislist[0].legend(labels=('Control - no RF', 'RF on'), fontsize=8)
        else:
            super(DressedStateCoupling, self)._plot(axislist)
            # norm_echo = 2. * (- data['counts'][:, 1] + data['counts'][:, 0]) / (data['counts'][:, 1] + data['counts'][:, 0])
            # norm_deer = 2. * (- data['counts'][:, 3] + data['counts'][:, 2]) / (data['counts'][:, 3] + data['counts'][:, 2])
            # axislist[0].hold(False)
            # axislist[0].plot(tau, norm_echo, 'b')
            ...
            axislist[0].legend(labels=('control up {:.0f}kcps'.format(np.mean(data['counts'][:, 1])), 'control down {:.0f}kcps'.format(np.mean(data['counts'][:, 0])), 'exp up {:.0f}kcps'.format(np.mean(data['counts'][:, 3])), 'exp down {:.0f}kcps'.format(np.mean(data['counts'][:, 2]))), fontsize=8)
            # axislist[0].set_xlabel('spin lock duration [ns]')
            axislist[0].set_title('Dressed state coupling between NV and dark spin \n mw-power:{:.0f}dBm, mw_freq:{:.3f} GHz, rf-power:{:.0f}dBm, rf_freq:{:.3f} MHz'.format(self.settings['mw_pulses']['mw_power'], self.settings['mw_pulses']['mw_frequency']*1e-9,self.settings['RF_pulses']['RF_power'], self.settings['RF_pulses']['RF_frequency']*1e-6))
    ...
    def _update_plot(self, axislist):
            ...
            if len(axislist[0].lines) == 0:
                self._plot(axislist)
                return
            super(DressedStateCoupling, self)._update_plot(axislist)
            axislist[0].legend(labels=('control up ',
                                       'control down ',
                                       'exp up ',
                                       'exp down'), fontsize=8)
